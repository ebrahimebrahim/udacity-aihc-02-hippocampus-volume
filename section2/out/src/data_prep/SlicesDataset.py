...
class SlicesDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        data_index, sagittal_slice_index = slc
        series_dict = self.data[data_index]
        sagittal_slice_img = series_dict['image'][sagittal_slice_index]
        sagittal_slice_seg = series_dict['seg'][sagittal_slice_index]
        sample['image'] = torch.from_numpy(sagittal_slice_img).type(torch.FloatTensor).unsqueeze(0)
        sample['seg'] = torch.from_numpy(sagittal_slice_seg).unsqueeze(0)
