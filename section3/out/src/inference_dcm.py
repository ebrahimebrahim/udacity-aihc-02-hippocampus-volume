...
def create_report(inference, header, orig_vol, pred_vol):
    ...
    draw.text((10, 0), "HippoVolume.AI (v1.0)", (255, 255, 255), font=header_font)
    draw.multiline_text((10, 90),
                        f"Patient ID: {header.PatientID}\n"+
                        f"Hippocampus Volume: {inference['total']}\n"+
    ...
    #
    thumbnail_size = 200
    class_to_rgba = {0:[0,0,0,0], 1:[0,200,0,196], 2:[128,128,0,196]}
    for slice_index,ypos in zip(slice_nums, range(280,1000,thumbnail_size+20)):
        slice = orig_vol[slice_index]
        slice_label = np.flip(pred_vol[slice_index]).T.astype(np.uint8)
        slice_label_rgba = np.zeros(slice_label.shape+(4,))
        for j in range(slice_label.shape[0]):
            for k in range(slice_label.shape[1]):
                slice_label_rgba[j,k] = class_to_rgba[slice_label[j,k]]
        slice_label_rgba = slice_label_rgba.astype(np.uint8)
        nd_img = np.flip((slice/np.max(slice))*0xff).T.astype(np.uint8)
        pil_i = Image.fromarray(nd_img, mode="L").convert("RGBA").resize((thumbnail_size,thumbnail_size))
        pil_i_label = Image.fromarray(slice_label_rgba, mode="RGBA").resize((thumbnail_size,thumbnail_size))
        pimg.paste(pil_i, box=(10, ypos))
        pimg.paste(pil_i_label, box=(10, ypos), mask=pil_i_label)
...
if __name__ == "__main__":
    ...
    os_command("storescu localhost 4242 -v -aec HIPPOAI "+report_save_path)
